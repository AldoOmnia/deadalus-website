...
Generates both themes:
  factory-brand.png       — top fades to white (light mode)
  factory-brand-dark.png  — top fades to black (dark mode)
"""
...
def build(page_color, out_name, dark=False):
    ...
    # Horizontal brand gradient (same stops as the logo)
    ...
    # Tritone: deep shadows go to black instead of full-saturation brand
    # color. This preserves the original curved silhouettes (bushes /
    # vignette) at the bottom so the image ends organically into black.
    shadow = np.clip(lum / 0.30, 0.0, 1.0) ** 1.4    # 0 for near-black source pixels
    out *= shadow[..., None]

    if dark:
        # Dark theme: highlights (sky / haze) roll off to black instead of
        # white, so the illustration reads as a gradient glow emerging from
        # the black page rather than a white image pasted on it.
        hi = np.clip((lum - 0.55) / 0.30, 0.0, 1.0) ** 1.15
        out *= (1.0 - hi)[..., None]
    ...
    # Fade the top into the page background for a seamless blend
    page = Image.new("RGB", (w, h), page_color)
    fade = np.clip(1.0 - yy / 0.38, 0.0, 1.0) ** 1.35        # opaque at row 0 -> clear by ~38%
    fade_img = Image.fromarray((fade * 255).astype(np.uint8).repeat(w, axis=1), "L")
    result = Image.composite(page, result, fade_img)

    # Gentle black assist at the very bottom only — the organic silhouettes
    # from the source (kept by the tritone) do most of the blending; this
    # just guarantees the last rows are pure black where the footer begins.
    ...
    result.save(out_name, optimize=True)
    print("saved", out_name, result.size)


build((255, 255, 255), "factory-brand.png")
build((0, 0, 0), "factory-brand-dark.png", dark=True)
